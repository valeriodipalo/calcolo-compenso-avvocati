...
/** Lookup table: slug → LucideIcon from toolRegistry */
const toolIconMap = new Map<string, LucideIcon>(
  TOOLS.filter((t) => t.ready).map((t) => [t.slug, t.icon])
);

function toolHref(slug?: string): string | null {
  return slug && readySlugs.has(slug) ? `/${slug}` : null;
}

/** Filter NAV_SECTIONS to only include live tools, non-empty clusters, non-empty sections */
function buildLiveSections() {
  return NAV_SECTIONS.map((section) => ({
    ...
    clusters: section.clusters
      .map((cluster) => ({
        ...cluster,
        tools: cluster.tools.filter((tool) => tool.slug && readySlugs.has(tool.slug)),
      }))
...
export function Navbar() {
  ...
  const activeSectionData = liveSections.find((s) => s.id === openSection) ?? null;
    ...
    <>
      ...
      {activeSectionData && (
        <>
          ...
          >
            <div className="max-w-[920px] mx-auto flex min-h-[260px]">
              {/* Cluster sidebar */}
                ...
                <div className="flex-1 p-5">
                  ...
                  <div className="grid grid-cols-2 gap-x-2 gap-y-0.5">
                    {activeClusterData.tools.map((tool) => {
                      ...
                      const ToolIcon = tool.slug ? toolIconMap.get(tool.slug) : undefined;
                      ...
                      return (
                        ...
                        >
                          {ToolIcon ? (
                            <ToolIcon className="w-3.5 h-3.5 shrink-0 text-[oklch(0.55_0.03_250)] group-hover:text-[oklch(0.75_0.10_85)] transition-colors" />
                          ) : (
                            <span
                              className="w-1.5 h-1.5 shrink-0 bg-[oklch(0.75_0.10_85)] group-hover:bg-[oklch(0.22_0.04_250)] transition-colors"
                      ...
                      <div key={cluster.id}>
                        ...
                        >
                          <div
                            ...
                            style={{ borderRadius: "0.375rem" }}
                          >
                            <ClusterIcon className={`w-3.5 h-3.5 ${cluster.iconFg}`} />
                          </div>
                            ...
                            {cluster.tools.map((tool) => {
                              ...
                              return (
                                <Link
                                  key={tool.label}
                                  href={href}
                                  className="flex items-center gap-2 py-2 text-[13px] text-[oklch(0.40_0.02_250)] hover:text-[oklch(0.20_0.02_250)] transition-colors"
                                  onClick={() => setDrawerOpen(false)}
                                >
                                  {ToolIcon ? (
                                    <ToolIcon className="w-3.5 h-3.5 shrink-0 text-[oklch(0.55_0.03_250)]" />
                                  ) : (
                                    <span className="w-3 h-px shrink-0 bg-[oklch(0.75_0.10_85)]" />
                                  )}
                                  <span>{tool.label}</span>
                                </Link>
                              );
