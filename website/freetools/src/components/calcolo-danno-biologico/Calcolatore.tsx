...
/**
 * Calcola il demoltiplicatore per età.
 * Micropermanenti: parte da età 10 (sotto i 10 anni = 1.0)
 * Milano/TUN: parte da età 0
 */
function getDemoltiplicatore(modalita: Modalita, eta: number): number {
  if (modalita === "micropermanenti") {
    return Math.max(0, 1 - Math.max(0, eta - 10) * 0.005);
  }
  return Math.max(0, 1 - eta * 0.005);
}

/**
 * Calcola la quota sofferenza Milano in base ai punti di invalidità.
 * 1-9 punti: 25% del biologico
 * 10-34 punti: dal 26% al 50% (progressione lineare)
 * 35-100 punti: 50% del biologico
 */
function getSofferenzaMilano(dannoBiologico: number, punti: number): number {
  if (punti <= 9) {
    return dannoBiologico * 0.25;
  }
  if (punti >= 35) {
    return dannoBiologico * 0.50;
  }
  // Progressione lineare da 26% (punti=10) a 50% (punti=34)
  const percentuale = 0.26 + ((punti - 10) / (34 - 10)) * (0.50 - 0.26);
  return dannoBiologico * percentuale;
}
...
function calcola(
  ...
  const demoltiplicatore = getDemoltiplicatore(modalita, eta);
  ...
  if (modalita === "micropermanenti") {
    const coefficienti = [1.0, 1.1, 1.2, 1.3, 1.5, 1.7, 1.9, 2.1, 2.3];
    const coefficiente = coefficienti[punti - 1] ?? 1;
    dannoPermanente = micropermanentiData.puntoBase2026 * coefficiente * demoltiplicatore;
    dannoITT = micropermanentiData.ittGiornaliero2026 * giorniITT;
    dannoITP = micropermanentiData.ittGiornaliero2026 * giorniITP * (percentualeITP / 100);
  } else if (modalita === "milano") {
    dannoPermanente = milanoData.puntoBiologico * punti * demoltiplicatore;
    dannoSofferenza = getSofferenzaMilano(dannoPermanente, punti);
    dannoITT = milanoData.ittGiornaliero100 * giorniITT;
    dannoITP = milanoData.ittGiornaliero100 * giorniITP * (percentualeITP / 100);
  } else {
    // TUN
    dannoPermanente = tunData.valoreBasePunto * punti * demoltiplicatore;
    dannoITT = tunData.ittGiornaliero * giorniITT;
    dannoITP = tunData.ittGiornaliero * giorniITP * (percentualeITP / 100);
  }

  const subtotale = dannoPermanente + dannoSofferenza + dannoITT + dannoITP;
    ...
    <div id="calcolatore" className="scroll-mt-24">
      <div className="bg-white border border-border shadow-lg overflow-hidden" style={{ borderRadius: "0.5rem" }}>
        ...
        <div className="bg-[oklch(0.25_0.04_250)] px-6 py-5 flex items-center gap-3">
          ...
          <h2 className="text-xl text-[oklch(0.90_0.01_85)] tracking-wide">Calcolatore Danno Biologico</h2>
        </div>
            ...
            <div className="border-t border-border pt-6 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <div className="bg-[oklch(0.97_0.005_250)] p-4" style={{ borderRadius: "0.375rem" }}>
                  ...
                  <p className="text-lg font-semibold text-[oklch(0.25_0.04_250)] font-mono">{formatCurrency(risultato.dannoPermanente)}</p>
                </div>
                {modalita === "milano" && (
                  <div className="bg-[oklch(0.97_0.005_250)] p-4" style={{ borderRadius: "0.375rem" }}>
                    <p className="text-xs font-semibold text-[oklch(0.50_0.02_250)] uppercase tracking-wider mb-1">Quota sofferenza</p>
              ...
              <p className="text-xs text-muted-foreground leading-relaxed">
                ...
                {modalita === "micropermanenti" && " Punto base 2025/2026: € 963,40 — coefficienti più che proporzionali (art. 139 CdA)."}
